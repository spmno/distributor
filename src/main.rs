...
fn main() {
    ...
    let study_upstream = "study";
    let study = Upstream::new(vec!["127.0.0.1:8889".to_string()]).unwrap();
    instance.add_service(GenBackgroundService::new(
        "study healt check".to_string(),
        study.lb.clone(),
    ));
    add_upstream(study_upstream, study);
    ...
    let server = Server::new(
        ...
        vec![
            Loation {
                ...
                upstream: study_upstream.to_string(),
                replace: true,
            },
            Loation {
